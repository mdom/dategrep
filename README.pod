...
Print all lines until DATESPEC exclusively. Default to the current time at programm start. See
L<VALID-DATE-FORMATS|https://metacpan.org/pod/distribution/Date-Manip/lib/Date/Manip/Date.pod#VALID-DATE-FORMATS>
for a list of possible formats for DATESPEC.
...
=head1 DELTAS FOR START AND END DATESPEC

Besides the formats listed in
L<VALID-DATE-FORMATS|https://metacpan.org/pod/distribution/Date-Manip/lib/Date/Manip/Date.pod#VALID-DATE-FORMATS>,
dategrep can handle deltas from any date by using the format I<DELTA
from DATE> as arguments to --start and --end:

  --start "1 hour ago from -15:00"
