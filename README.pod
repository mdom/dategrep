...
At the moment dategrep will die as soon as it finds a line that is not parsable.
In a future version this will be configurable.

dategrep also expects the files to be sorted. If the time serie is not
ascending dategrep might be exiting before the last line in its date range is
printed.
